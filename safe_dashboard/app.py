...
def load_gdf():
    gdf = gpd.read_file(
        "data/shapefile/Cincinnati_Statistical_Neighborhood_Approximations_(SNA).shp"
    )
...
with tab2:
    ...
    # Identify which column in the shapefile holds the neighborhood name
    sna_col_candidates = ["SNA_NAME", "SNA_Name", "NHOOD", "NAME", "SNA"]
    sna_col = next((c for c in sna_col_candidates if c in gdf.columns), None)

    if sna_col is None:
        st.error(
            f"Could not find a neighborhood name column in the shapefile. "
            f"Available columns: {list(gdf.columns)}"
        )
    else:
        gdf_m = (
            gdf[[sna_col, "geometry"]]
            .merge(df_latest, left_on=sna_col, right_on="SNA_NAME", how="left")
            .reset_index(drop=True)
        )
        gdf_m["_id"] = gdf_m.index

        geojson = json.loads(gdf_m.to_json())
        for feat in geojson["features"]:
            feat["id"] = feat["properties"]["_id"]
